                            ...
                            <div id="td_name">
                                <h2>{{tdDtl.Name}}
                                    <span v-if="tdDtl.Star === true">
                                        <img src="~@/assets/like_white.svg" alt="like" title="like" class="icon status" @click="onStarChange(tdDtl)">
                                    </span>
                                        <span v-else>
                                    <img src="~@/assets/unlike_white.svg" alt="unlike" title="unlike" class="icon status" @click="onStarChange(tdDtl)">
                                    </span>
                                </h2>
                                <!--お気に入り-->
                            </div>
                            <div id="icon_btn">
                                ...
                                <span id="btn_test" v-if="tdDtl.TestDescriptionResult == null || tdDtl.TestDescriptionResult.Summary ==='ERR' || tdDtl.TestDescriptionResult.Summary ==='NA'">
                                    ...
                                    <router-link :to="{ name: 'TestDescriptionCopy'}" class="icon">
                                        <img src="~@/assets/copy_white.svg" alt="copy" title="copy" class="icon" @click='postHistory_testDescriptionCopy($route.name,tdDtl.Id)'>
                                    </router-link>
                                    <a href="#" class="icon">
                                        <img src="~@/assets/delete_white.svg" alt="delete" title="delete" class="icon" @click="testDescriptionDelete">
                                    </a>
                                </template>
                                <template v-else>
                                <span id="btn_test" v-if="tdDtl.TestDescriptionResult == null || tdDtl.TestDescriptionResult.Summary ==='ERR' || tdDtl.TestDescriptionResult.Summary === 'NA'">
                            ...
                            <!-- エラー情報 -->
                                <template v-if="tdDtl.TestDescriptionResult">
                                    <template v-if="tdDtl.TestDescriptionResult.Summary ==='ERR'">
                                        <div id="error_info">
                                            <label class="error_title">{{$t("testDescriptionDetail.errorInfo")}}</label>
                                            <table class="dataTable">
                                                <tr id="error_code">
                                                    <td>{{$t("testDescriptionDetail.errorCode")}}</td>
                                                    <td>
                                                ...
                                                <tr id="error_message">
                                                    <td>{{$t("testDescriptionDetail.errorMessage")}}</td>
                                                    <td><span style="white-space:pre-wrap; word-wrap:break-word;">{{tdErrorMessage}}</span></td>
                                                </tr>
                                                <tr id="error_detail">
                                                    <td>{{$t("testDescriptionDetail.errorDetail")}}</td>
                                                    <td><span style="white-space:pre-wrap; word-wrap:break-word;">{{tdDtl.TestDescriptionResult.Detail}}</span></td>
                                                </tr>
                                            </table>
                                        </div>
                                    </template>
...
<!--                                            <label for="Panel1_1">+ {{$t("testDescriptionDetail.qualityMeasurement")}}</label>-->
                                            <label for="Panel1_1">
                                                <span v-if="accordionOnOffFlag.Panel1_1" title="unfold" ><img class="icon" title="unfold" :src="lessInfo"></span>
                                                <span v-else><img class="icon" :src="moreInfo"></span>
                                                {{$t("testDescriptionDetail.qualityMeasurement")}}
                                            </label>
                                            <input type="checkbox" id="Panel1_1" class="on_off" v-model="accordionOnOffFlag.Panel1_1" />
                                            <ul>
                                                <li>
                                                    <table class="dataTable">
                                                        <tr>
                                                        ...
                                                        <template v-if="qualityMeasurement.Enable">
                                                            ...
                                                            <tr>
                                                                <td class="tbl21">{{$t("testDescriptionDetail.condition")}}</td>
                            ...
                            <div id="acd_right" v-if="tdDtl.TestDescriptionResult.Summary === 'OK' || tdDtl.TestDescriptionResult.Summary === 'NG'">
                                <div id="opinion" class="accordion">
                                    ...
                                    <div class="opinion">
                                        ...
                                        <label class="con">{{$t("testDescriptionDetail.opinion")}}</label>

                                        <textarea name="example" cols="80" rows="6" v-model="changeOpinion"></textarea>
                                        <div id="btn_reg">
                                            <template v-if="$i18n.locale === 'en'">
                    ...
                    <div id="acd_bottom">
                        <hr/>
                        <!-- TD詳細 -->
                        <div id="td_detail" class="eachInfo">
                            ...
                            <div class="accordion detail" v-if="tdDtl.TestDescriptionResult == null || tdDtl.TestDescriptionResult.Summary === 'ERR' || tdDtl.TestDescriptionResult.Summary === 'NA'">
                                ...
                                <table class="dataTable">
                                    <tr>
                                        <td>{{$t("testDescriptionDetail.qualityDimension")}}</td>
                                        <td>{{tdDtl.QualityDimension.Name}}</td>
                                    </tr>
                                    <tr>
                                        <td></td>
                                        <td></td>
                                    ...
                                    <template v-if="qualityMeasurement.Enable">
                                        <tr>
                                            <td>{{$t("testDescriptionDetail.measurement")}}</td>
                                            <td>{{qualityMeasurement.Name}}</td>
                                        </tr>
                                        <tr>
                                            <td>{{$t("testDescriptionDetail.description")}}</td>
                                            <td>{{qualityMeasurement.Description}}</td>
                                        </tr>
                                        <tr>
                                            <td>{{$t("testDescriptionDetail.condition")}}</td>
                                            <td v-if="relationalOperators">
                                                {{qualityMeasurement.Name}} {{getRelationalOperator(qualityMeasurement.RelationalOperatorId)}} {{qualityMeasurement.Value}}
                                            </td>
                                        </tr>
                                        <tr>
                            ...
                            <!-- TargetInventory -->
                            <div id="target_inventory" class="eachInfo">
                                <label>{{$t("testDescriptionDetail.targetInventory")}}</label>

                                <table class="dataTable">
                                    <tr>
                                        <td>{{$t("testDescriptionDetail.inventoryDefine")}}</td>
                                        <td class="underline">{{$t("testDescriptionDetail.inventoryRegistrationName")}} ({{$t("testDescriptionDetail.inventoryRegistrationFilePath")}})</td>
                                    </tr>
                                    <tr v-for="targetInv of tdDtl.TargetInventories" :key="targetInv.Id">
                                        <td>{{targetInv.TemplateInventoryName}}</td>
                                        <td>{{targetInv.Name}} ({{targetInv.FilePath}})</td>
                                    </tr>
                                </table>
                            </div>
                            <!-- AIT -->
                            <div id="ait" class="eachInfo">
                                ...
                                <table class="dataTable">
                                    ...
                                    <tr>
                                        <td class="tbl21">{{$t("testDescriptionDetail.create_user_account")}}</td>
                                        <td class="tbl22">{{tdDtl.TestRunner.CreateUserAccount}}</td>
                                    </tr>
                                    <tr>
                                        <td class="tbl21">{{$t("testDescriptionDetail.create_user_name")}}</td>
                                        <td class="tbl22" v-if="tdDtl.TestRunner.CreateUserName">
                                            {{tdDtl.TestRunner.CreateUserName}}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="tbl21">{{$t("testDescriptionDetail.quality")}}</td>
                                        <td class="tbl22">
                                            <img v-if="tdDtl.TestRunner.Quality" src="~@/assets/new_window.svg" alt="new-window" title="new-window" class="new-window">
                                            <a v-bind:href=cleanUrl(tdDtl.TestRunner.Quality) target="_blank">{{cleanUrl(tdDtl.TestRunner.Quality)}}</a>
                                        </td>
                                    </tr>
                                    <tr>
...
export default {
    ...
    mounted: function () {
        ...
        this.$axios.get(url)
            .then((response) => {
                ...
                // install_mode
                this.install_mode = this.tdDtl.TestRunner.InstallMode;
    ...
    methods: {
        onOffAllAccordion() {
            this.accordionOnOffFlag.Panel1_1 = this.onOffAllFlag;
        },
        //言語データ読み込み
        setLanguageData() {
        ...
        onSelectedRowsChange(params) {
            ...
            if(params.selectedRows.length <= 0){
                this.isActivated = true; // addボタン非活性
                this.preview_visible = false; //プレビュー非表示
                this.preview_row_index = -1; //プレビュー対象のidをダミー値に
        ...
        //addボタンの活性判定
        checkAddBTNActive(){
            this.isActivated = true; //非活性状態で初期化
            ...
            for (let j = 0; j < this.graph_list.length; j++) {
                ...
                if(!registered){
                    this.isActivated = false; //右欄に未登録なら活性状態
        ...
        postReportGenerator(command) {
            ...
            if (this.errorMessages.length === 0) {
                ...
                this.$axios.post(url, this.requestBody, config)
                    .then((response) => {
